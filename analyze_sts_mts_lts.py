...
def compute(df):
    ...
    out["Close"]=df["Close"]; out["Open"]=df["Open"]; out["Volume"]=df["Volume"]
    sma5=SMAIndicator(df["Close"],5).sma_indicator()
    sma20=SMAIndicator(df["Close"],20).sma_indicator()
    sma60=SMAIndicator(df["Close"],60).sma_indicator()
    sma120=SMAIndicator(df["Close"],120).sma_indicator()
    rsi=RSIIndicator(df["Close"],14).rsi()
    macd=MACD(df["Close"],26,12,9)
    bb=BollingerBands(df["Close"],20,2)
    conv,base,spana,spanb=ichimoku(df)
    obv20=obv(df); vol20=df["Volume"].rolling(20).mean()
    out=out.assign(SMA5=sma5,SMA20=sma20,SMA60=sma60,SMA120=sma120,
                   RSI14=rsi,MACD=macd.macd(),MACD_SIG=macd.macd_signal(),MACD_HIST=macd.macd_diff(),
                   BB_M=bb.bollinger_mavg(),BB_U=bb.bollinger_hband(),BB_L=bb.bollinger_lband())
    out["BB_STD"]=(out["BB_U"]-out["BB_M"])/2
    out["ICH_CONV"]=conv; out["ICH_BASE"]=base; out["ICH_SPANA"]=spana; out["ICH_SPANB"]=spanb
    out["OBV"]=obv20; out["VOL20"]=vol20
    # scoring
    def sts(i):
...
def run(market):
    ...
    for t in tickers:
        try:
            df=yf.download(t, period="420d", interval="1d", auto_adjust=True, progress=False)
            ...
            print("ERR", t, e)
    if not rows: return
    rep_dir=os.path.join("reports",market); os.makedirs(rep_dir, exist_ok=True)
    rep=pd.DataFrame(rows).sort_values(["BUY_CANDIDATE","STS","MTS","LTS","close"],ascending=[False,False,False,False,False])
    ...
    lines=[f"[{market.upper()} Summary — {today}]",
           f"총 종목: {len(rep)}",
    ...
    if not sell.empty:
        ...
        lines.append("")
    open(os.path.join(rep_dir,f"{today}_summary.txt"),"w",encoding="utf-8").write("\n".join(lines))
